        ...
        if not export_excel_comp and not export_pdf_comp:
            ...
            if df_filtered_comparison.empty:
                    ...
                    with st.spinner(get_text('generating_comparison_excel')):
                        try:
                            excel_success_comp = export_comparison_report(
                                ...
                                comparison_config,
                                comparison_mode,
                                comparison path_dict['comparison_output_file']
                                )
                        except Exception as e:
                            excel_success_comp = False
                            st.error(f"❌ Lỗi khi xuất Excel: {e}")
                    if excel_success_comp:
                        st.success(get_text('comparison_excel_generated').format(os.path.basename(comparison path_dict['comparison_output_file'])))
                ...
                if export_pdf_comp:
                    ...
                    if pdf_success_comp:
                        st.success(get_text('comparison_pdf_generated').format(os.path.basename(comparison patch_dict['comparison_pdf_report'])))
                        report_generated_comp = True
                    else:
                ...
                # ======= HIỆN NÚT TẢI PDF/EXCEL SAU KHI XUẤT =========
                    with st.expander("📥 Tải báo cáo PDF/Excel so sánh"):
                        if export_excel_comp and os.path.exists(comparison_path_dict["comparison_output_file"]):
                            with open(comparison path_dict["comparison_output_file"], "rb") as f_excel:
                                st.download_button(
                                    label="📄 Tải Excel So sánh",
                                    data=f_excel,
                                    file_name=os.path.basename(comparison path_dict["comparison_output_file"]),
                                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                                    use_container_width=True,
                                    key="exp_excel_comp_btn"
                                )
                        if export_pdf_comp and os.path.exists(comparison_path_dict["comparison_pdf_report"]):
                            with open(comparison path_dict["comparison_pdf_report"], "rb") as f_pdf:
                                st.download_button(
                                    label="🖨️ Tải PDF So sánh",
                                    data=f_pdf,
                                    file_name=os.path.basename(comparison path_dict["comparison_pdf_report"]),
